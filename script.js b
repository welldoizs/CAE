(() => {
  ...
  const schoolEmailMessage = `Use um e-mail válido no formato nomealeatorio${SCHOOL_EMAIL_DOMAIN}.`;
  ...
  const errorMessage = (error) => {
    ...
    if (/CAE_SCHOOL_EMAIL_REQUIRED/i.test(raw)) return schoolEmailMessage;
