(() => {
  ...
  function subscribeRealtime() {
    ...
    state.realtime = state.supabase.channel(`cae-${state.user.id}`)
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: TABLE.posts }, () => { if (state.currentView === 'mural-view') queueMuralReload(); })
  ...
  async function loadAdmin(){
    ...
    try{
      ...
      $('admin-report-list').innerHTML=reportRows?.length?reportRows.map(report=>{
        ...
        const messageHTML = message ? `<div class="notice" style="margin:10px 0"><div class="cae-post-meta">Mensagem de ${escapeHTML(publicTagForUser(message.user_id))} · ${escapeHTML(formatDate(message.created_at))}</div><p>${escapeHTML(message.body).replace(/\n/g,'<br>')}</p>${message.deleted_at?'<div class="cae-muted">Esta mensagem já está ocultada.</div>':''}</div>` : '<div class="notice" style="margin:10px 0">A mensagem denunciada não pôde ser localizada.</div>';
        return `<article class="card"><div class="cae-post-meta">Denúncia · ${escapeHTML(formatDate(report.created_at))}</div><p><b>Motivo:</b> ${escapeHTML(report.reason)}</p>${messageHTML}<div class="actions"><button class="btn danger" type="button" data-hide-reported="${escapeHTML(report.message_id)}" data-report-id="${escapeHTML(report.id)}">Ocultar mensagem</button><button class="btn secondary" type="button" data-resolve-report="${escapeHTML(report.id)}">Resolver sem ocultar</button></div></article>`;
