(() => {
  ...
  const errorMessage = (error) => {
    ...
    if (/cae_invalid_vent_visibility/.test(combined)) return 'A privacidade escolhida é inválida. Recarregue a página e tente novamente.';
  ...
  const CHAT_REDACT_TERMS = [
    ...
    'merda','bosta,'merdinha','merdao','merdão','porra','caralho','cacete','foda','fodase','foda-se',
  ...
  const VENT_VISIBILITIES = Object.freeze({
    public: { label: 'Público', description: 'Depois de aprovado, fica disponível para todos na área Desabafos.' },
    private: { label: 'Privado', description: 'Fica visível somente para você e para administradores. Moderadores e outros estudantes não têm acesso.' }
  });

  function setVentVisibility(visibility) {
    const next = VENT_VISIBILITIES[visibility] ? visibility : 'public';
    state.ventVisibility = next;
    $$('[data-vent-visibility]').forEach(button => {
      const active = button.dataset.ventVisibility === next;
      button.classList.toggle('active', active);
      button.setAttribute('aria-pressed', active ? 'true' : 'false');
    });
    const helper = $('vent-visibility-help');
    if (helper) helper.textContent = VENT_VISIBILITIES[next].description;
  }

  async function finishVent(type, body = null, feeling = null, audioPath = null) {
    if (!state.user) return false;
    if (!body && !audioPath) return false;
    const visibility = VENT_VISIBILITIES[state.ventVisibility] ? state.ventVisibility : 'public';
    ...
    await loadHistory();
    $$('.compose').forEach(el => el.classList.add('hidden'));
    toast(visibility === 'private'
      ? 'Desabafo privado enviado. Somente administradores poderão acessá-lo.'
      : 'Desabafo enviado para moderação. Obrigado por confiar no CAE.');
    return true;
  }
  ...
  async function loadPublicVents() {
    ...
    try {
      ...
      const { data, error } = await state.supabase
        .from(TABLE.vents)
        .select('id,user_id,type,body,feeling,audio_url,created_at,status,visibility')
        .eq('status', 'approved')
  ...
  async function loadHistory() {
    ...
    try {
      ...
      list.innerHTML=data.map(vent=>{
        ...
        const visibility=String(getField(vent,['visibility'],'public'))==='private'?'private':'public';
        const statusHTML=`<span class="cae-status cae-status-${escapeHTML(status)}">${escapeHTML(statusLabels[status]||status)}</span>`;
        const privacyHTML=visibility==='private'
          ? '<span class="cae-privacy-badge private"><i data-lucide="lock-keyhole"></i>Privado · só administradores</span>'
          : '<span class="cae-privacy-badge public"><i data-lucide="globe-2"></i>Público após aprovação</span>';
        return `<article class="card">
          <div class="eyebrow">${escapeHTML(labels[type]||'Desabafo')} · ${escapeHTML(formatDate(vent.created_at))}</div>
          <div class="cae-post-meta" style="margin-top:8px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">${statusHTML}${privacyHTML}</div>
  ...
  async function loadAdmin(){
    ...
    try{
      ...
      const {data:pendingVentRows,error:pendingVentError}=await state.supabase.from(TABLE.vents).select('*').eq('status','pending').eq('visibility','public').order('created_at',{ascending:true}).limit(100);
      ...
      const pendingVentHTML=(pendingVentRows||[]).map(vent=>{
        ...
        const visibility=String(getField(vent,['visibility'],'public'))==='private'?'private':'public';
        const privacyHTML=visibility==='private'
          ? '<span class="cae-privacy-badge private"><i data-lucide="lock-keyhole"></i>Privado · acesso administrativo</span>'
          : '<span class="cae-privacy-badge public"><i data-lucide="globe-2"></i>Público após aprovação</span>';
        const canModerateThisVent=visibility==='private'?isAdmin():isStaff();
        const actionsHTML=canModerateThisVent
          ? `<div class="actions"><button class="btn primary" type="button" data-approve-vent="${escapeHTML(vent.id)}">Aprovar</button><button class="btn secondary" type="button" data-reject-vent="${escapeHTML(vent.id)}">Recusar</button><button class="btn danger" type="button" data-delete-vent="${escapeHTML(vent.id)}" data-audio-path="${escapeHTML(audioPath)}">Excluir</button></div>`
          : '<div class="notice"><b>Acesso administrativo:</b> este é um desabafo privado. Somente administradores podem acessá-lo.</div>';
        return `<article class="card"><div class="cae-post-meta" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">${escapeHTML(ventLabels[String(vent.type)]||'Desabafo')} · ${escapeHTML(publicTagForUser(vent.user_id))} · ${escapeHTML(formatDate(vent.created_at))} ${privacyHTML}</div>${feeling?`<div class="notice" style="margin-top:10px">${escapeHTML(feeling)}</div>`:''}${body?`<p>${escapeHTML(body).replace(/\n/g,'<br>')}</p>`:''}${audioHTML}${actionsHTML}</article>`;
      }).join('');
      const adminVentList=$('admin-vent-list');
      if(adminVentList) adminVentList.innerHTML=pendingVentHTML||'<div class="card"><p>Nenhum desabafo público aguardando moderação.</p></div>';

      const privateAdminCard=$('admin-private-vents-card');
      const privateAdminList=$('admin-private-vent-list');
      if (privateAdminCard) privateAdminCard.classList.toggle('hidden', !isAdmin());
      if (privateAdminList) {
        if (!isAdmin()) {
          privateAdminList.innerHTML = '';
        } else {
          const {data:privateVentRows,error:privateVentError}=await state.supabase.from(TABLE.vents).select('*').eq('visibility','private').order('created_at',{ascending:false}).limit(100);
          if(privateVentError)throw privateVentError;
          await preloadPublicProfiles((privateVentRows||[]).map(vent=>vent.user_id));
          const privateStatusLabels={pending:'aguardando moderação',approved:'aprovado · continua privado',rejected:'recusado'};
          const privateVentHTML=(privateVentRows||[]).map(vent=>{
            const body=getField(vent,['body'],'');
            const feeling=getField(vent,['feeling'],'');
            const audioPath=getField(vent,['audio_url'],'');
            const status=String(getField(vent,['status'],'pending'));
            const type=String(getField(vent,['type'],'free'));
            const statusHTML=`<span class="cae-status cae-status-${escapeHTML(status)}">${escapeHTML(privateStatusLabels[status]||status)}</span>`;
            const audioHTML=audioPath?`<div class="admin-audio"><div class="cae-muted">Áudio privado</div><button class="btn secondary" type="button" data-admin-play-audio="${escapeHTML(audioPath)}"><i data-lucide="play"></i>Ouvir áudio</button><audio class="admin-audio-player hidden" controls></audio></div>`:'';
            const actions=status==='pending'
              ? `<div class="actions"><button class="btn primary" type="button" data-approve-vent="${escapeHTML(vent.id)}">Aprovar</button><button class="btn secondary" type="button" data-reject-vent="${escapeHTML(vent.id)}">Recusar</button><button class="btn danger" type="button" data-delete-vent="${escapeHTML(vent.id)}" data-audio-path="${escapeHTML(audioPath)}">Excluir</button></div>`
              : `<div class="actions"><button class="btn danger" type="button" data-delete-vent="${escapeHTML(vent.id)}" data-audio-path="${escapeHTML(audioPath)}">Excluir</button></div>`;
            return `<article class="card"><div class="cae-post-meta" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><span>${escapeHTML(ventLabels[type]||'Desabafo')}</span><span>·</span><span>${escapeHTML(publicTagForUser(vent.user_id))}</span><span>·</span><span>${escapeHTML(formatDate(vent.created_at))}</span><span class="cae-privacy-badge private"><i data-lucide="lock-keyhole"></i>Privado · somente admin</span>${statusHTML}</div>${feeling?`<div class="notice" style="margin-top:10px">${escapeHTML(feeling)}</div>`:''}${body?`<p>${escapeHTML(body).replace(/\n/g,'<br>')}</p>`:''}${audioHTML}${actions}</article>`;
          }).join('');
          privateAdminList.innerHTML=privateVentHTML||'<div class="card"><p>Nenhum desabafo privado foi enviado.</p></div>';
        }
      }
