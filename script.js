(() => {
  ...
  const SCHOOL_EMAIL_DOMAIN = '@escola.pr.gov.br';
  ...
  const schoolEmailMessage = `Use um e-mail escolar terminado em ${SCHOOL_EMAIL_DOMAIN}.`;
  ...
  const errorMessage = (error) => {
    ...
    if (/cae_school_email_required/.test(combined)) return schoolEmailMessage;
    if (/cae_user_banned/.test(combined)) return 'Esta conta está bloqueada pela moderação.';
    if (/cae_user_timeout/.test(combined)) return 'Sua conta está em timeout e não pode enviar mensagens até o prazo terminar.';
    if (/cae_chat_low_value/.test(combined)) return 'A mensagem parece automática/genérica demais para o chat. Escreva de forma mais direta e pessoal.';
    if (/cae_chat_blocked/.test(combined)) return 'Essa mensagem não pode ser enviada pelo filtro automático do CAE.';
    if (/cae_chat_spam/.test(combined)) return 'Evite repetir mensagens ou enviar muitas mensagens seguidas. Aguarde um pouco.';
    if (/cae_chat_too_long/.test(combined)) return 'A mensagem é longa demais.';
    if (/profile_not_found/.test(combined)) return 'A conta existe, mas seu perfil do CAE ainda não foi criado. Tente entrar novamente.';
    if (/target_user_not_found/.test(combined)) return 'Nenhuma conta foi encontrada com esse ID público.';
    if (/self_moderation_not_allowed/.test(combined)) return 'Você não pode aplicar essa ação na própria conta.';
    if (/only_admin/.test(combined)) return 'Somente administradores podem alterar cargos.';
    if (/invalid_role/.test(combined)) return 'Cargo inválido.';
    if (/invalid_moderation_action/.test(combined)) return 'Ação de moderação inválida.';
    if (/invalid_timeout/.test(combined)) return 'Escolha um timeout entre 1 minuto e 7 dias.';

    // O Supabase não diferencia com segurança "e-mail inexistente" de "senha errada".
    // Por isso o CAE exibe uma mensagem útil sem revelar se a conta existe.
    if (/invalid[_ ]login[_ ]credentials|invalid[_ ]credentials|invalid[_ ]grant|user not found|wrong password|incorrect password/.test(combined)) {
      return 'Não foi possível entrar: e-mail ou senha incorretos.';
    }
    if (/email not confirmed|email_not_confirmed/.test(combined)) return 'Seu e-mail ainda não foi confirmado. Confira sua caixa de entrada e tente novamente.';
    if (/email.*invalid|invalid.*email|email_address_invalid/.test(combined)) return 'Digite um endereço de e-mail válido.';
    if (/already registered|user already registered|user_already_exists/.test(combined)) return 'Este e-mail já possui uma conta. Tente entrar em vez de criar outra.';
    if (/password.*6|at least 6/.test(combined)) return 'A senha precisa ter pelo menos 6 caracteres.';
    if (/rate limit|too many requests|over_email_send_rate_limit/.test(combined)) return 'Muitas tentativas em pouco tempo. Aguarde alguns segundos e tente novamente.';
    if (/too many attempts|captcha/.test(combined)) return 'O acesso foi temporariamente limitado por segurança. Aguarde um pouco e tente novamente.';
    if (/row-level security|rls|not allowed|permission denied/.test(combined)) return 'O banco bloqueou essa ação. Verifique as políticas RLS do CAE.';
    if (/relation .* does not exist|table .* does not exist/.test(combined)) return 'Uma tabela do CAE ainda não existe no Supabase.';
    if (/column .* does not exist/.test(combined)) return 'Uma coluna do CAE está diferente do esperado no Supabase.';
    if (/cae_poll_votes_one_per_user|duplicate key.*poll/.test(combined)) return 'Você já votou nessa enquete.';
    if (/cae_post_reactions|duplicate key/.test(combined)) return 'Essa reação já está registrada.';
    if (/failed to fetch|networkerror|load failed|err_name_not_resolved|name_not_resolved|dns|502|bad gateway|503|service unavailable|504|gateway timeout/.test(combined)) return 'O CAE não conseguiu alcançar o servidor do Supabase. Confirme se o projeto está ativo e tente novamente.';
    if (/project.*paused|paused.*project|540/.test(combined)) return 'O projeto do Supabase está pausado. Abra o Dashboard e retome o projeto.';
    return raw;
  };

  // Filtro do chat: cobertura ampla de vulgaridades, insultos e linguagem de ódio.
  ...
  const CHAT_REDACT_TERMS = [
    // Vulgaridades / palavrões em PT-BR
      // Profanidade / xingamentos / expressões vulgares
    'vsf','vsfd','v.s.f','v.s.f.d','vtnc','vtmnc','vtnc','vai tomar no cu',
    'vai tomar no olho do cu','vai tomar no rabo','vai pra puta que pariu',
    'vai pra pqp','vai à puta que pariu','vai para pqp','vai se lascar',
    'vai se danar','vai se ferrar','vai pro inferno','vai para o inferno',
    'foda-se','fodase','foda se','foda-se mesmo','foda essa merda',
    'foda-se você','foda-se vc','fodam-se','fodamse','fode-se','fodaço',
    'fodão','fodao','fodona','fodaço','fudança','fudido','fudida',
    'caralho','caralhada','caralhudo','caralhuda','caralhão','caralhao',
    'caralhote','caralho velho','cacete','caceta','cacetada','caceteiro',
    'caceteira','merda','merdagem','merdoso','merdosa','merdeiro','merdeira',
    'merdalhão','merdalhao','merdento','merdenta','merda nenhuma',
    'porra nenhuma','que porra','essa porra','puta merda','puta que o pariu',
    'pqp','p.q.p','pqpariu','pqpqp','vsf','vai se fuder','vai se fuder mesmo',
    'se fuder','se fode','fode-se','fodam-se','fodeu-se','fudendo tudo',
    'fudência','fudencia','fuder','fudê','fude','fudeu','fudesse',

    // Insultos / humilhações comuns
    'babaca','babacão','babacao','babacona','babacazinho','babacazinha',
    'idiota','idiotão','idiotao','idiotice','imbecil','imbecilidade',
    'otário','otario','otária','otaria','otarião','otariano','otariota',
    'palhaço','palhaco','palhaça','palhaca','palhacinho','palhacinha',
    'trouxa','trouxão','trouxaço','mané','mane','manézão','manezao',
    'cretino','cretina','cretinão','cretinaço','asno','anta','jumento',
    'jegue','mula','burro','burra','burro demais','animal','animalaço',
    'animalão','imbecilóide','tapado','tapada','lerdo','lerda','lerdão',
    'lerdao','lesado','lesada','mongolão','mongolao','retardado mental',
    'retardada mental','débil','debil','débilóide','debiloide','estúpido',
    'estupido','estúpida','estupida','estupidez','cabeça de vento',
    'cabeça oca','cabeça vazia','cabeça de bagre','cabeça de merda',
    'sem noção','sem nocao','sem cérebro','sem cerebro','sem cérebro nenhum',
    'lerdo de merda','burro de merda','idiota de merda','imbecil de merda',
    'otário de merda','otario de merda','palhaço de merda','palhaco de merda',
    'fracassado','fracassada','fracassinho','fracassinho de merda',
    'inútil','inutil','inútil pra nada','inutil pra nada','peso morto',
    'traste','traste humano','escória','escoria','ralé','rale','gentinha',
    'lixo','lixo humano','lixo ambulante','verme','verme humano','parasita',
    'parasita inútil','parasita social','praga','praga humana','peste',
    'peste humana','desprezível','desprezivel','ridículo','ridiculo',
    'ridícula','ridicula','patético','patetico','patética','patetica',
    'nojento','nojenta','asqueroso','asquerosa','repugnante','imundo','imunda',
    'porco','porca','porcalhão','porcalhao','porcalhona','imprestável',
    'imprestavel','miserável','miseravel','canalha','cretinóide','cretinoide',
    'mala sem alça','mala sem alca','pé no saco','pe no saco','chato',
    'chata','chatão','chatao','insuportável','insuportavel','babão','babao',
    'puxa-saco','puxa saco','lambe-botas','lambe botas','lambe saco',
    'otáriozinho','otariozinho','idiotinha','imbecilzinho','babacão',

    // Insultos relacionados a sexualidade / moralidade
    'prostituta','prostituto','prostituto de merda','prostituta de merda',
    'meretriz','meretrizinha','cafetão','cafetaão','cafeteiro','cafetina',
    'gigolô','gigolo','gigoló','gigolo de merda','corno manso','corno safado',
    'corno otário','corno otario','chifrudo','chifruda','galinha','galinheiro',
    'galinha velha','galinha de quintal','devassa','devasso','sem-vergonha',
    'sem vergonha','sem-vergonha nenhuma','safado de merda','safada de merda',
    'tarado','tarada','taradão','taradao','taradona','pervertido',
    'pervertida','pervertidão','pervertidaço','libertino','libertina',
    'promíscuo','promiscua','promíscua','promiscuo','vulgar','imoral',
    'imoralzinho','sem caráter','sem carater','mau caráter','mau carater',
    'mau-caráter','mau-carater',

    // Termos sexuais / obscenos
    'bunda','bundão','bundao','bunduda','bundudo','rabão','rabao','rabo',
    'rabeta','rabudo','rabuda','ânus','anus','ânus sujo','anus sujo',
    'orifício','orificio','orifício anal','orificio anal','nádegas',
    'nadegas','boga','bogue','boceta','bucetão','bucetao','bucetinha',
    'bucetona','bucetuda','xerequinha','xerecão','xerecao','xerequinha',
    'xoxotinha','xoxotão','xoxotao','piranha','piranha velha','piranha de merda',
    'grelo','grelinha','clitóris','clitoris','safadeza','putaria',
    'putaria pesada','sacanagem','sacanagem pura','sacaneta','sacanagem',
    'punheta','punheteiro','punheteira','punhetinha','punhete','punhetão',
    'punhetão','punhetaço','siririca','siriqueira','sirizeira','broxar',
    'broxa','broxado','broxada','broxante','broxão','broxao','tesão',
    'tesao','taradice','gozo','gozando','gozador','gozadora','ejaculação',
    'ejaculacao','esperma','porra de esperma','sêmen','semen','leitinho',
    'pau duro','pau mole','rola dura','rola mole','cacete duro','pinto pequeno',
    'pinto mole','pintinho','pintão','pintao','peruzinho','peru','bengala',
    'jeba','jibóia','jiboia','jebaço','jebaco','negócio','minhoca',

    // Ofensas com partes do corpo
    'seu cu','seu rabo','seu pau','sua bunda','sua pica','sua boceta',
    'sua buceta','sua rola','sua xota','seu saco','sua cara de cu',
    'cara de cu','cara de bunda','cara de pau','cara de pica',
    'cara de merda','cara de ovo','cara de tacho','cara de cavalo',
    'cara de jumento','cara de porco','cara de anta','cara de otário',
    'cara de otario','cabeça de cu','cabeça de pau','cabeça de merda',
    'olho do cu','olho do rabo','filho do capeta','filha do capeta',

    // Insultos com aparência / inteligência / caráter
    'feio','feia','feioso','feiosa','horroroso','horrorosa','monstro',
    'monstra','aberração','aberracao','aberração humana','aberração ambulante',
    'deformado','deformada','esquisito','esquisita','ridículo','ridicula',
    'nojento de merda','porco imundo','porca imunda','fedido','fedida',
    'fedorento','fedorenta','catinga','catinguento','catinguenta',
    'burro pra caralho','burra pra caralho','lento','lenta','lerdo',
    'lerda','tapado','tapada','tanso','tansa','pamonha','pamonha de merda',
    'banana','banana de pijama','orelha de abano','nariz de batata',
    'cabeça de abacate','cabeça de vento','miolo mole','miolo de pão',
    'sem futuro','sem serventia','sem talento','sem capacidade',
    'incompetente','incompetenta','fracote','fracotão','fracotao',

    // Ameaças / agressões verbais
    'vai pro inferno','vai para o inferno','vai pro quinto dos infernos',
    'vai para o quinto dos infernos','que inferno','inferno','maldito',
    'maldita','malditinho','malditinha','desgraça ambulante',
    'praga ambulante','suma daqui','cai fora','vaza','some daqui',
    'desaparece','vai embora seu lixo','morre','morram','morra','morram todos',
    'tomara que morra','tomara que se foda','tomara que se dane',
    'bem feito','se lasca','se ferra','ferra-te','danou-se','danado',
    'danada','capeta','capetão','capetao','demônio','demonio','demoninho',
    'diabo','diabinho','filho do diabo','filha do diabo','cria do diabo',

    // Abreviações / internetês
    'pqp','p.q.p','pqpariu','fdp','f.d.p','f d p','vsf','vsfd','vtnc',
    'vtmnc','vtnc','tnc','tnmnc','tmnc','tmncvc','se fudeu','sfudeu',
    'sfdr','sfd','fml','wtf','stfu','gtfo','f.u','fu','f off','fuck',
    'fuck you','fucking','fucked','motherfucker','mother fucker',
    'bullshit','shit','shithead','dumbass','asshole','bastard','bitch',
    'son of a bitch','piece of shit','dickhead','prick','jerk','douche',
    'douchebag','scumbag','jackass','crap','damn','goddamn','bloody hell',

    // Ofensas em inglês
    'bitch','bitches','son of a bitch','sonofabitch','motherfucker',
    'motherfucking','fuck','fucking','fucked','fuckface','fuckhead',
    'fuckwit','ass','asshole','arsehole','dumbass','smartass','badass',
    'dick','dickhead','dickwad','douche','douchebag','prick','twat',
    'cunt','wanker','wank','jerkoff','jerk-off','shit','shithead',
    'shitface','bullshit','horseshit','dipshit','jackass','dumbshit',
    'craphead','scumbag','bastard','bastards','son of a bitch',
    'piece of shit','piece of crap','screw you','screw off','piss off',
    'pissed off','goddamn','damned','damn you','bloody bastard',

    // Slurs / termos pejorativos adicionais
    'homo','hominho','boiola de merda','viado de merda','bicha de merda',
    'maricas de merda','sapatão de merda','sapatona de merda',
    'traveco de merda','traveca','travecona','transviado','invertido',
    'aberração sexual','aberração de gênero','aberração de genero',
    'cracker','honky','redneck','hillbilly','yid','heeb','kike',
    'gook','chink','spic','wetback','beaner','sandnigger','raghead',
    'towelhead','paki','gypsy','chink','ching chong','jigaboo',
    'darkie','negro imundo','preto nojento','branco imundo','judeu safado',
    'judeu nojento','árabe imundo','arabe imundo','asiático imundo',
    'asiatico imundo','índio imundo','indio imundo','latino imundo',

    // Termos pejorativos / discriminatórios comuns
    'retard','retarded','moron','idiot','imbecile','stupid','stupid bitch',
    'dumb','dumbass','dumbfuck','moronic','loser','freak','weirdo',
    'creep','creepy','degenerate','degenerado','degenerada','depravado',
    'depravada','sub-humano','subhumano','sub-humana','subhumano',
    'parasite','vermin','scum','trash','human trash','disgrace',

    // Neonazismo / extremismo / supremacismo
    'heil hitler','sieg heil','white power','white pride','white supremacy',
    'white supremacist','white supremacism','nazi','nazista','nazismo',
    'neo nazi','neo-nazi','neonazi','neonazista','aryan pride',
    'aryan power','blood and soil','blood & soil','88','1488','14/88',
    'fourteen words','ss','ss runes','sieg heil','heil','hitler',
    'hitlerjugend','hitler youth','lebensraum','jude','juden','juden raus',
    'rassenschande','zigeunersau','gas the jews','gas them',

    // Insultos compostos muito usados em chats
    'filho de uma égua','filho de egua','filho de jumento','filho de burro',
    'filho de uma mula','filha de uma égua','filha de egua',
    'seu animal','seu verme','seu parasita','seu lixo','seu traste',
    'seu miserável','seu miseravel','seu desgraçado','seu desgracado',
    'seu arrombado','sua arrombada','seu canalha','sua canalha',
    'seu escroto','sua escrota','seu imundo','sua imunda',
    'seu porco','sua porca','seu nojento','sua nojenta',
    'seu otário','seu otario','sua otária','sua otaria',
    'seu idiota','sua idiota','seu imbecil','sua imbecil',
    'seu babaca','sua babaca','seu palhaço','sua palhaça',
    ...
    'putao','putaria','puta que pariu','filho da puta','filho de puta','filho de uma puta','fdp','f.d.p',
    'f d p','filhadaputa','vadia','vadiazinha','piranha','biscate','vagabunda','vagabundo','desgraça',
    'desgraca','desgraçado','desgracado','desgraçada','desgracada','arrombado','arrombada','escroto',
    'escrota','safado','safada','canalha','nojento','nojenta','corno','corna','rola','rolão','rolao',
    'pica','pinto','piroca','buceta','boceta','xoxota','xota','xereca','perereca','cona','mamada','mamar',
    'gozar','gozada','chupa','chupada','chupando','lambe rola','pau','saco','sacana',
    // Insultos / humilhações comuns
    'babaca','babacão','babacao','otario','otário','otaria','otária','idiota','imbecil','burro','burra',
    'jumento','anta','asno','cretino','cretina','trouxa','palhaço','palhaco','palhaça','palhaca',
    'mané','mane','mongoloide','mongol','débil mental','debil mental','retardado','retardada','retarda',
    'inútil','inutil','lixo humano','escória','escoria','fracassado','fracassada','nojento','nojenta',
    // Termos de ódio / slurs (inclui formas em inglês que aparecem em chats brasileiros)
    'nigger','nigga','nigro','niggah','faggot','fag','dyke','tranny','cracker','spic','chink','kike',
    'coon','monkey','macaco','macaca','preto imundo','negro imundo','judeu imundo','jap imundo',
    'viado','veado','bicha','bixona','traveco','maricas','marica','sapatão','sapatao','sapatona','boiola',
    // Neonazismo / supremacismo usados como insulto ou propaganda
    'heil hitler','white power','white pride','nazi','nazista','skinhead nazi','kkk',
  ];
  ...
  function containsNormalizedTerm(normalized, term) {
    ...
    // Detecta tentativas como "p u t a" depois da normalização.
    const compact = t.replace(/\s+/g, '');
    if (compact.length >= 3) {
      for (let i=0;i<tokens.length;i++) {
        let run = '';
        for (let j=i;j<Math.min(tokens.length, i+compact.length);j++) {
          if (!/^[a-z0-9]$/.test(tokens[j])) break;
          run += tokens[j];
          if (run === compact) return true;
          if (run.length >= compact.length) break;
        }
      }
    }
    return false;
  ...
  function moderateChatText(rawBody) {
    ...
    if (CHAT_REDACT_TERMS.some(term => containsNormalizedTerm(normalized, term))) {
      for (const pattern of CHAT_REDACT_PATTERNS) text = text.replace(pattern, '###');
      const after = normalizeModerationText(text);
      if (CHAT_REDACT_TERMS.some(term => containsNormalizedTerm(after, term))) {
  ...
  async function login(event) {
    ...
    if (!validSchoolEmail(email)) { setStatus('login-status', schoolEmailMessage); $('login-email')?.focus(); return; }
    if (!password) { setStatus('login-status', 'Digite sua senha.'); $('login-password')?.focus(); return; }
    ...
    try {
      ...
      if (/invalid[_ ]login[_ ]credentials|invalid[_ ]credentials|invalid[_ ]grant/i.test(String(error?.code || error?.message || ''))) $('login-password')?.focus();
  ...
  async function hydrateSession(session, force = false) {
    ...
    try {
      ...
      prepareFeelings();
      await Promise.allSettled([loadChat(), loadMural(), loadPublicVents(), loadHistory()]);
      subscribeRealtime();
      ...
      try { await state.supabase.auth.signOut(); } catch (signOutError) { console.warn('HYDRATE SIGNOUT', signOutError); }
      state.user = null;
      state.profile = null;
      state.session = null;
      state.lastHydratedAccessToken = null;
      showAuth();
      setAuthMode('login');
      setStatus('login-status', `Não foi possível carregar sua conta no CAE: ${message}`);
  ...
  function showView(id) {
    ...
    if (id === 'public-vents-view') loadPublicVents();
  ...
  function subscribeRealtime() {
    ...
    state.realtime = state.supabase.channel(`cae-${state.user.id}`)
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: TABLE.vents }, () => { if (state.currentView === 'public-vents-view') loadPublicVents(); if (state.currentView === 'admin-view' && isStaff()) loadAdmin(); })
  ...
  async function loadPublicVents() {
    const list = $('public-vent-list');
    if (!list || !state.user || !state.supabase) return;
    try {
      list.innerHTML = '<div class="card"><p class="muted">Carregando os desabafos da comunidade…</p></div>';
      const { data, error } = await state.supabase
        .from(TABLE.vents)
        .select('id,user_id,type,body,feeling,audio_url,created_at,status')
        .eq('status', 'approved')
      ...
      const vents = data || [];
      await preloadPublicProfiles(vents.map(vent => vent.user_id));
      const labels = { free: 'Texto livre', guided: 'Perguntas guiadas', feelings: 'Sentimentos', audio: 'Áudio' };
      const audioSources = await Promise.all(vents.map(async vent => {
        if (!vent.audio_url) return '';
        const result = await state.supabase.storage.from(AUDIO_BUCKET).createSignedUrl(vent.audio_url, 600);
        if (result.error) {
          console.warn('PUBLIC AUDIO URL', result.error);
          return '';
        }
      ...
      if (!vents.length) {
        list.innerHTML = '<div class="card public-vent-empty"><div class="eyebrow">Ainda estamos começando</div><h3>Nenhum desabafo público ainda.</h3><p class="cae-muted">Quando um desabafo for aprovado pela moderação, ele aparece aqui — separado do Mural, para a comunidade encontrar com facilidade.</p><div class="actions" style="justify-content:center;margin-top:14px"><button class="btn primary" type="button" data-open-view="desabafo-view"><i data-lucide="pen-line"></i>Ser a primeira voz</button></div></div>';
        refreshIcons();
        return;
      }

      list.innerHTML = vents.map((vent, index) => {
        const body = getField(vent, ['body'], '');
        const feeling = getField(vent, ['feeling'], '');
        const audioUrl = audioSources[index];
        const audioHTML = audioUrl
          ? `<div class="cae-public-audio"><audio controls preload="none" src="${escapeHTML(audioUrl)}" style="width:100%"></audio></div>`
          : (vent.audio_url ? '<div class="notice">O áudio aprovado não pôde ser carregado agora. Tente novamente mais tarde.</div>' : '');
        const canDelete = vent.user_id === state.user.id || isStaff();
        const deleteHTML = canDelete
          ? `<div class="actions" style="margin-top:12px"><button class="btn danger" type="button" data-delete-vent="${escapeHTML(vent.id)}" data-audio-path="${escapeHTML(vent.audio_url || '')}"><i data-lucide="trash-2"></i>Remover desabafo</button></div>`
          : '';
        return `<article class="card public-vent-card">
          <div class="cae-post-meta"><span>${escapeHTML(publicTagForUser(vent.user_id))}</span><span>·</span><span>${escapeHTML(labels[String(vent.type)] || 'Desabafo')}</span><span>·</span><span>${escapeHTML(formatDate(vent.created_at))}</span></div>
          ${feeling ? `<div class="public-vent-feeling">${escapeHTML(feeling)}</div>` : ''}
          ${body ? `<p class="public-vent-body">${escapeHTML(body).replace(/\n/g,'<br>')}</p>` : ''}
          ${audioHTML}
          <div class="public-vent-footer"><span class="public-vent-approved"><i data-lucide="shield-check"></i>Aprovado pela moderação</span><span class="cae-muted">ID público · sem e-mail</span></div>
          ${deleteHTML}
        </article>`;
      }).join('');
      refreshIcons();
    } catch (error) {
      console.error('PUBLIC VENTS', error);
      list.innerHTML = `<div class="card"><p class="cae-danger">${escapeHTML(errorMessage(error))}</p><div class="actions" style="margin-top:10px"><button class="btn secondary" type="button" data-open-view="public-vents-view">Tentar novamente</button></div></div>`;
    }
  }
  ...
  async function deleteVent(id, audioPath = null) {
    ...
    try {
      ...
      await loadHistory();
      if (state.currentView === 'public-vents-view') await loadPublicVents();
      if (isStaff()) await loadAdmin();
    } catch (error) {
  ...
  function bindEvents(){
    ...
    $('login-email')?.addEventListener('input',()=>{ if ($('login-status')?.textContent) setStatus('login-status',''); });
    $('login-password')?.addEventListener('input',()=>{ if ($('login-status')?.textContent) setStatus('login-status',''); });
