(() => {
  ...
  function subscribeRealtime() {
    ...
    state.realtime = state.supabase.channel(`cae-${state.user.id}`)
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: TABLE.vents }, () => { if (state.currentView === 'mural-view') queueMuralReload(); if (state.currentView === 'admin-view' && isStaff()) loadAdmin(); })
  ...
  async function loadPublicVents() {
    ...
    try {
      ...
      if (!vents.length) {
        list.innerHTML = '<div class="card"><p>Nenhum desabafo aprovado foi publicado ainda.</p><p class="cae-muted">Quando um desabafo for aprovado pela moderação, ele aparecerá aqui para a comunidade.</p></div>';
        return;
      }
      ...
      list.innerHTML = `<div class="card"><p class="cae-danger">${escapeHTML(errorMessage(error))}</p></div>`;
    }
  }
  ...
  async function loadMural() {
    ...
    try {
      const tasks = [loadAnnouncements(),loadEvents(),loadPosts(),loadPolls(),loadPublicVents()];
