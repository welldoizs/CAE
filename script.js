(() => {
  ...
  function showView(id) {
    ...
    if (id === 'desabafo-view') removeDailyVentUI();
  ...
  function removeDailyVentUI() {
    document.querySelectorAll('#daily-message, #daily-block-message').forEach(el => el.remove());
    const subtitle = document.querySelector('#desabafo-view .section-title .subtle');
    if (subtitle) subtitle.textContent = 'Você decide o que escrever ou gravar.';
  ...
  async function createPoll(event) {
    ...
    try {
      ...
      const {error:optionError}=await state.supabase.from(TABLE.pollOptions).insert(options.map(option_text=>({poll_id:poll.id,text:option_text,option_text})));
      ...
      if(pollId){
        await state.supabase.from(TABLE.polls).delete().eq('id',pollId).then(({error:cleanupError})=>{
          if(cleanupError) console.warn('POLL CLEANUP',cleanupError);
        }).catch(cleanupError=>console.warn('POLL CLEANUP',cleanupError));
      }
      toast(errorMessage(error),'error');
    }finally{setBusy(button,false);}
  }
  ...
  async function init(){
    if(state.initialized)return;state.initialized=true;bindEvents();removeDailyVentUI();refreshIcons();
    if(!ensureClient())return;
    initAuthListener();
    let sessionResult;
    try {
